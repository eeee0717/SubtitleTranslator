...
public partial class TencentProviderViewModel : ViewModelBase
{
  ...
  public TencentProviderViewModel()
  {
  ...
  [RelayCommand]
  private void ShowSecretId()
  {
    IsSecretIdShow = !IsSecretIdShow;
    if (IsSecretIdShow)
    {
      Application.Current!.TryFindResource("EyeShowRegular", out var res);
      SecretIdButtonIcon = res as StreamGeometry ?? StreamGeometry.Parse(StreamGeometryNotFound);
    }
    else
    {
  ...
  {
    IsSecretKeyShow = !IsSecretKeyShow;
    if (IsSecretIdShow)
    {
      Application.Current!.TryFindResource("EyeShowRegular", out var res);
