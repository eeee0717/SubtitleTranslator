...
public partial class YoudaoProviderViewModel: ViewModelBase
{
  [ObservableProperty] private bool _isAppIdShow;
  [ObservableProperty] private StreamGeometry _appIdButtonIcon;
  [ObservableProperty] private string? _appId;
  [ObservableProperty] private bool _isAppKeyShow;
  [ObservableProperty] private StreamGeometry _appKeyButtonIcon;
  ...
  public YoudaoProviderViewModel()
  {
    Application.Current!.TryFindResource("EyeShowRegular", out var res);
    var streamGeometry = res as StreamGeometry ?? StreamGeometry.Parse(StreamGeometryNotFound);
    AppIdButtonIcon = streamGeometry;
    AppKeyButtonIcon = streamGeometry;
    ConfigFileHelper configFileHelper = new();
    AppId = configFileHelper.ProviderOptions?.YoudaoProviderOptions.AppId;
    AppKey = configFileHelper.ProviderOptions?.YoudaoProviderOptions.AppKey;
  }
  ...
  {
    IsAppIdShow = !IsAppIdShow;
    if (IsAppIdShow)
    {
      Application.Current!.TryFindResource("EyeShowRegular", out var res);
      AppIdButtonIcon = res as StreamGeometry ?? StreamGeometry.Parse(StreamGeometryNotFound);
    }
    else
    {
  ...
  {
    IsAppKeyShow = !IsAppKeyShow;
    if (IsAppIdShow)
    {
      Application.Current!.TryFindResource("EyeShowRegular", out var res);
      AppKeyButtonIcon = res as StreamGeometry ?? StreamGeometry.Parse(StreamGeometryNotFound);
    }
    else
    {
      Application.Current!.TryFindResource("EyeHideRegular", out var res);
  ...
  {
    YoudaoTranslator youdaoTranslator = new();
    TestResult = await youdaoTranslator.Translate("hello", "en", "zh-CHS");
  }
}